...
function iterator( alpha, s, m, options ) {
	...
	if ( !isPositiveNumber( alpha ) ) {
		throw new TypeError( format( '0Pk7P', alpha ) );
	}
	if ( !isPositiveNumber( s ) ) {
		throw new TypeError( format( '0Pk7T', s ) );
	}
	if ( !isNumber( m ) || isnan( m ) ) {
		throw new TypeError( format( '0Pk4O', m ) );
	}
	if ( arguments.length > 3 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pk2h', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pk35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
