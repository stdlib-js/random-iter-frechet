...
tape( 'the function throws an error if scale parameter `s` is not a positive number', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			iterator( 2.0, value, 1.0 );
		};
	}
});

tape( 'the function throws an error if location parameter `m` is not a number', function test( t ) {
	var values;
	var i;
...
tape( 'the function returns an iterator protocol-compliant object (seed)', function test( t ) {
	...
	it1 = iterator( 2.0, 4.0, -1.0, {
		'seed': seed
	});
	it2 = iterator( 2.0, 4.0, -1.0, {
		'seed': seed
	});
...
tape( 'attached to the returned iterator is the underlying PRNG', function test( t ) {
	var it = iterator( 2.0, 5.0, 2.0 );
	t.equal( typeof it.PRNG, 'function', 'has property' );

	it = iterator( 2.0, 5.0, 2.0, {
...
tape( 'the function returns an iterator for generating pseudorandom numbers from a Fréchet distribution', function test( t ) {
	var rand;
	var it;
	var i;

	// Note: we assume that the underlying generator is the following PRNG...
	rand = frechet( 2.0, 5.0, 3.0, {
		'seed': 12345
	});
